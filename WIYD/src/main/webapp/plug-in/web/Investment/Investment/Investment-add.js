  $(function() {
    ...
    $("#scoreShow").html("该类文的下载积分为:"+$(".radio_classify:checked").data('score')+"分");
     ...
     $('.radio_classify').on('change ifChecked',function () {
      //切换表单
      //切换显示所需分数
    	$("#scoreShow").html("该类文的下载积分为:"+$(".radio_classify:checked").data('score')+"分");
    	
       $('.form_classify').addClass('element-invisible');
...
//    var uploader2=createUploader("#uploader2",params2);
  ...
  function addFile(type){
  	...
  	if(type==1){
  		data={
			 'ucode':$("#ucode1").val(),
	    	 'categoryId':$(".radio_classify:checked").val(),
	    	 'documentDetail':$("#documentDetail").val(),
  		...
  		data={
			 'ucode':$("#ucode2").val(),
	    	 'categoryId':$(".radio_classify:checked").val(),
	    	 'documentDetail':$("#questionDescribe").val(),
	    	 'documentName':$("#questionTitle").val()
  		};
  	}
  	$.ajax({
		      type: "POST",
		      url: path+"/documentFileController/addDocumentFile.do",
		      data: data,
		      async: false,
		      error: function(request) {
		        layer.msg("提交失败");
  ...
  switch (state) {
    ...
    case 'finish':
      ...
      if (stats.successNum) {
        ...
        //----------------------------------------对上传完成进行额外操作修改
        if($("#documentName").val()=='undefined'||$("#documentName").val()==''){
