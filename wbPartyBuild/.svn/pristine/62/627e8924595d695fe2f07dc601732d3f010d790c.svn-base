...
public class DynamicTask {
	...
	public synchronized static void updateSpringMvcTaskXML(CronTrigger trigger, String cronExpression) {
		...
		try {
			URI url = DynamicTask.class.getClassLoader().getResource("applicationContext-quartz.xml").toURI();
			file = new File(url.getPath());
			document = saxReader.read(new FileInputStream(file));
		} catch (Exception e) {
			logger.error("读取系统中用到的SQL 语句XML出错");
			throw new RuntimeException("---------读取applicationContext-quartz.xml文件出错:" + e.getMessage());
		}
