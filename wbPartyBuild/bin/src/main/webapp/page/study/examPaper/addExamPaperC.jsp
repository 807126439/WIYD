...
<body>
	<div class="pd-20" style="text-align: center;">
		<form action="<%=path%>/examPaperController/addExamPaper.do" method="post" class="form form-horizontal" id="form-examPaperC-add">
			...
			<div class="row cl" style="margin-top: 20px;">
				...
				<div class="formControls col-xs-3 col-sm-2">
					<input type="text" class="input-text" name="examMinute"  datatype="n" nullmsg="请填写考试时间" style="width:80%;margin-right:2px">分钟
				</div>
			</div>

		 	<input id="choosentopic" name="choosentopic" type="hidden"> 
		 			...
		 			<span class="select-box" style="width:160px;">
						<select class="select" name="categoryId" size="1" id="categoryId">	
							...
							<c:forEach items="${categoryList}" var="c">
								...
								<option value="${c.id}">${c.cateName}</option>s
							</c:forEach>
						</select>
					</span> 
					<span class="select-box" style="width:160px;">
		...
		<script type="text/javascript" src="<%=path%>/plug-in/web/scripts/study/examPaper/addExamPaperC.js"></script>
