...
function selectAnswer() {
  ...
  if ($quest.hasClass('singlechoice') || $quest.hasClass('trueorfalse')) { 
	  //单选和判断点击后自动调用sendAnswer方法
...
function sendAnswer() {
  ...
  $.ajax({
      ...
      success: function(data) { 
          ...
          $quest.find('.btn-submit').addClass('disabled');
      }
...
function countDown(obj) {
  ...
  var timeEnd = obj.timeEnd || null; //时间结束时触发
  var timeFollow = obj.timeFollow || null; //跟随计数器每段时间触发1次
